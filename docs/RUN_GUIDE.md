...
| `videos_dir` | **解析対象のmp4が入っているディレクトリ**（新規ジョブごとに書き換える） |
| `out_dir` | **Stage1-3の出力先ベースディレクトリ**（動画ごとに`out_dir/<動画名>/`が作られる） |
...
（CLI指定があればそちらが優先。両方省略した場合はエラーになる）。
...
`<新しいジョブ名>.yaml`を開いて`videos_dir`と`out_dir`を書き換える
